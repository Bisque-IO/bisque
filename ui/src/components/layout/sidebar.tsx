...
export function Sidebar() {
  ...
  const toggle = useCallback(() => {
    setCollapsed((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("bisque_sidebar_collapsed", String(next));
      } catch {
        /* noop */
      }
      return next;
          ...
          {collapsed ? (
            ...
            <>
              <div className="flex items-center gap-2 pl-1">
                <LogoIcon className="h-7 w-7" />
                <span className="text-lg font-semibold tracking-tight translate-y-px">
                  bisque
                </span>
              </div>
              <Tooltip>
        ...
        {collapsed ? (
          <div className="flex-1 overflow-y-auto py-4 flex flex-col items-center">
            <nav className="flex flex-col items-center gap-4">
              {navItems.map((item, i) => {
                if ("divider" in item) {
                  return <div key={i} className="my-1 mx-auto w-5 border-t" />;
                }
                  ...
                  <Tooltip key={item.to}>
                    <TooltipTrigger asChild>
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          cn(
                            "size-9 flex items-center justify-center rounded-md transition-colors shrink-0",
                            isActive
