...
use super::{IntoInner, IoBuf, IoBufMut, SetLen, VectoredSlice};
use crate::t_alloc;
...
pub trait IoVectoredBuf: 'static {
    ...
    ///
    /// ```
    /// use maniac::buf::{IoBuf, IoVectoredBuf, VectoredSlice};
    ///
    /// # fn main() {
...
pub trait IoVectoredBufMut: IoVectoredBuf + SetLen {
    ...
    ///
    /// ```
    /// use maniac::buf::{IoBuf, IoVectoredBuf, IoVectoredBufMut, VectoredSlice};
    ///
    /// # fn main() {
    /// /// Create a buffer with given content and capacity.
    /// fn new_buf(slice: &[u8], cap: usize) -> Vec<u8> {
