...
use crate::util::bits::find_nearest_by_distance;
...
impl ContractGroupInner {
    fn new(
        ...
        Self {
            ...
            force_summary,
            sub_tree_mask,
            signal_tree_shift: signal_tree_capacity.trailing_zeros(),
            signal_tree_mask: signal_tree_capacity - 1,
            signal_trees,
            available_trees,
            contracts,
            thread_counter: AtomicU64::new(0),
            stopped: AtomicBool::new(false),
            tree_summary: TreeSummary::new(sub_tree_count as usize),
    ...
    fn get_tree_and_signal_index(&self, index: u32) -> (u64, u64) {
        ...
        (
            index >> self.signal_tree_shift,
            index & self.signal_tree_mask,
        )
    }
    ...
    fn set_contract_signal(&self, index: u32) {
        ...
        if tree_was_empty {
            let (was_set, was_empty) = self.tree_summary.set_bit(tree_index);
    ...
    fn set_contract_signal_yielded(&self, index: u32) {
        ...
        if tree_was_empty {
            let (_was_set, _was_empty) = self.tree_summary.set_bit(tree_index);
        }
    ...
    pub fn get_available_contract(&self, bias: &mut BiasState) -> Option<u32> {
        ...
        for i in 0..self.available_trees.len() * 36 {
            let sub_tree_index = counter.wrapping_add(i as u64) & sub_tree_mask;
            // let sub_tree_index = counter.wrapping_add(64) & sub_tree_mask;
            ...
            if !self.available_trees[sub_tree_index as usize].empty() {
                ...
                if signal_index != INVALID_SIGNAL_INDEX {
                    println!(
                        ...
                        ((sub_tree_index * 64) + signal_index) as u32,
                        sub_tree_index,
                        signal_index
                    );
                    bias.alloc_counter = counter.wrapping_add(1);
                    let index = (sub_tree_index << self.signal_tree_shift) + signal_index;
    ...
    fn poll_next_contract_with_bias_inner(
        ...
        // if self.force_summary {
        //     for _ in 0..self.signal_tree_mask {
        //         // for _ in 0..1 {
        //         let result = self.poll_next_with_summary_selection(bias, was_woken);
        //         if result.is_some() {
        //             return result;
        //         }
        //     }
        ...
        // } else if self.force_hybrid || self.signal_trees.len() > Self::SUMMARY_SELECTION_THRESHOLD {
        //     self.poll_next_hybrid(bias, was_woken)
        // } else {
        //     self.poll_next_linear_scan(bias, was_woken)
        // }

        // if let Some(result) = self.poll_next_hybrid(bias, was_woken) {
        //     return Some(result);
        // }
        // self.poll_next_linear_scan(bias, was_woken)
        // self.poll_next_hybrid(bias, was_woken)
        ...
        // }
        // self.poll_next_linear_scan(bias, was_woken)
        // self.poll_sequential(bias, was_woken)
        // if let Some(result) = self.poll_partition_scan(bias, was_woken) {
        //     return Some(result);
        // }
        // None
        self.poll_next_linear_scan(bias, was_woken)
    ...
    #[inline]
    fn poll_partition_scan(&self, bias: &mut BiasState, was_woken: bool) -> Option<&Contract> {
        // crate::util::random::rapidrng_fast(&mut bias.summary_bias);
        // crate::util::random::rapidrng_fast(&mut bias.flags);
        // crate::util::random::rapidrng_fast(&mut bias.hint);

        // let num_summaries = self.tree_summary.word_count as u64;
        // let num_summaries_mask = num_summaries - 1;

        let mut total_threads = self.thread_counter.load(Ordering::Relaxed);
        ...
        let signal_tree_capacity = 1u64 << self.signal_tree_shift;
        let signal_tree_mask = signal_tree_capacity - 1;

        let num_trees = self.signal_trees.len() as u64;
        let num_contracts = self.contracts.len() as u64;

        let mut partition_size = num_contracts / total_threads;
        let thread_id = bias.thread_id;
        let partition_offset = thread_id * partition_size;

        // Last thread gets the remainder
        if thread_id + 1 == total_threads {
            partition_size = num_contracts - partition_offset;
        }
        ...
        // let partition_size = num_contracts;
        // let partition_offset = 0;
        // let partition_size = num_contracts;

        for i in 0..partition_size {
            // bias.poll_count = crate::util::random::next_u64();
            // Get bias-based starting position within partition, then iterate
            let contract_id = partition_offset + ((bias.cycle) % partition_size);
            // let contract_id = bias.cycle & (num_contracts - 1);

            let sub_tree_index = contract_id / 64;
            let signal_index = contract_id & 63;

            if sub_tree_index >= self.signal_trees.len() as u64 {
                // bias.cycle += 1;
                bias.cycle = sub_tree_index * 64 + 64;
                continue;
            }

            // sub_tree_index = partition_offset + ((bias.poll_count % partition_size);

            // sub_tree_index = partition_offset + (sub_tree_index % partition_size);

            let signal = &self.signal_trees[sub_tree_index as usize];
            let signal_value = signal.load_relaxed();
            ...
            // crate::util::random::rapidrng_fast(&mut bias.poll_count);
            if signal_value & (0x8000000000000000 >> signal_index) == 0 {
                bias.cycle += 1;

                // let signal_index_ = crate::util::bits::select_bit_bidirectional(
                //     signal_value,
                ...
                // bias.cycle = sub_tree_index * 64 + signal_index + 1;
            ...
            // crate::util::random::rapidrng_fast(&mut bias.poll_count);
            // bias.poll_count += 1;
            // let (signal_index_, new_hint) = crate::signal_tree::select_64(
            //     crate::util::random::next_u64(),
            //     signal_value,
            //     signal_index,
            // );

            let (acquired, tree_is_empty) = signal.try_claim(signal_index);
            if !acquired {
                // bias.cycle += 1;
                bias.cycle = sub_tree_index * 64 + 64;
                continue;
            ...
            bias.poll_count += 1;
            if bias.poll_count & 63 == 0 {}
            bias.cycle += 1;

            // bias.poll_count += 1;

            // XOR-based bias advancement (matches work_contract)
            // let x = new_hint ^ bias.flags;
            // let b = (x & (!x).wrapping_add(1)) & signal_tree_mask;
            // if b == 0 {
            //     // bias.flags = (sub_tree_index + 1) << self.signal_tree_shift;
            ...
            if tree_is_empty {
                self.tree_summary.clear_bit(sub_tree_index);
                if !self.signal_trees[sub_tree_index as usize].empty() {
                    self.tree_summary.set_bit(sub_tree_index);
                }
            }
    ...
    #[inline]
    fn poll_next_with_summary_selection(
        &self,
        bias: &mut BiasState,
        was_woken: bool,
    ) -> Option<&Contract> {
        // bias.flags = crate::util::random::next_u64();
        // crate::util::random::rapidrng_fast(&mut bias.summary_bias);
        // crate::util::random::rapidrng_fast(&mut bias.flags);
        // crate::util::random::rapidrng_fast(&mut bias.hint);
        // bias.summary_bias = crate::util::random::next_u64();

        // Match work_contract pattern: use bias.flags directly
        let signal_tree_capacity = 1u64 << self.signal_tree_shift;
        let signal_tree_mask = signal_tree_capacity - 1;
        // let target_tree = (bias.summary_bias >> self.signal_tree_shift) & self.sub_tree_mask;

        // Find a tree with work, starting from target_tree
        let (sub_tree_index, _) = self.tree_summary.select_with_bias(bias);

        if sub_tree_index == u64::MAX {
            return None;
        }

        let (signal_index, tree_is_empty, new_hint) =
            self.signal_trees[sub_tree_index as usize].select_with_hint(bias.flags, bias.hint);

        if signal_index == INVALID_SIGNAL_INDEX {
            return None;
        }

        let index = ((sub_tree_index << self.signal_tree_shift) + signal_index) as u32;
        ...
        // Clear summary bit if tree became empty.
        // After clearing, re-check tree and restore bit if work appeared (race recovery).
        if tree_is_empty {
            self.tree_summary.clear_bit(sub_tree_index);
            // Re-check: if tree now has work, another thread added it after our select.
            // Restore the summary bit so it won't be missed.
            if !self.signal_trees[sub_tree_index as usize].empty() {
                self.tree_summary.set_bit(sub_tree_index);
            }
        }
        ...
        let contract = &self.contracts[index as usize];
        if self.process_contract(index, contract) {
            // Inject randomness periodically to prevent long-term bias patterns
            // bias.maybe_inject_randomness();
            // Chained wakeup: if we were woken and tree still had work, wake another
            if !tree_is_empty && self.blocking && was_woken {
                self.semaphore.release();
            }
            return Some(contract);
        }
        // Another worker took this contract - continue searching
        None
    }
    ...
    fn poll_next_linear_scan(&self, bias: &mut BiasState, was_woken: bool) -> Option<&Contract> {
        ...
        for _ in 0..self.signal_trees.len() {
            ...
            if self.force_summary {
                ...
                if self.tree_summary.words[summary_index as usize].load(Ordering::Acquire) == 0 {
                    // if summary_value == 0 {
            ...
            let signal_value = self.signal_trees[sub_tree_index as usize].load();

            if signal_value != 0 {
                // if true {
                let (signal_index, tree_is_empty, new_hint) = self.signal_trees
                    [sub_tree_index as usize]
                    .select_with_hint(bias.flags, bias.hint);
            ...
            if self.force_summary
                && crate::util::random::rapidrng_fast(&mut bias.summary_hint) & 511 == 0
                && self.tree_summary.words[summary_index as usize].load(Ordering::Acquire) == 0
            {
                self.tree_summary.clear_bit(sub_tree_index);
                if !self.signal_trees[sub_tree_index as usize].empty() {
                    self.tree_summary.set_bit(sub_tree_index);
            ...
            // bias.hint = new_hint;
            sub_tree_index += 1;
            // sub_tree_index += crate::util::random::next_u64() & sub_tree_mask;
            bias.flags = sub_tree_index * 64;
            // bias.flags = (crate::util::random::next_u64() & sub_tree_mask) * 64;
        }
