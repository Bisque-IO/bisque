...
///
/// ```ignore
/// use maniac::fs::OpenOptions;
/// use maniac::driver::IoPool;
///
/// let pool = IoPool::new()?;
/// let file = OpenOptions::new().read(true).open(&pool, "foo.txt")?;
/// ```
///
...
impl OpenOptions {
    ...
    ///
    /// The file will be associated with the given [`IoPool`] for async IO.
    pub fn open(&self, pool: &IoPool, path: impl AsRef<Path>) -> io::Result<File> {
        self.0.open(pool, path)
    }
}
