...
/// writes are guaranteed to be intact. In particular you should be aware that
/// writes using [`write_all`] are not guaranteed to occur as a single write,
/// so multiple threads writing data with [`write_all`] may result in
/// interleaved output.
///
