...
use super::{AsyncWrite, AsyncWriteAt, IoResult};
...
/// Macro for looping over vectored write operations.
#[macro_export]
macro_rules! loop_write_vectored {
    ($buf:ident, $iter:ident, $write_expr:expr) => {{
        ...
        loop {
            if $iter.buf_len() > 0 {
                return $write_expr.await.into_inner();
            }
...
pub trait AsyncWriteExt: AsyncWrite {
    ...
    async fn write_all<T: IoBuf + Send>(&mut self, mut buf: T) -> BufResult<(), T> {
        loop_write_all!(
            buf,
            buf.buf_len(),
            needle,
    ...
    async fn write_vectored_all<T: IoVectoredBuf + Send>(
        &mut self,
        mut buf: T,
    ) -> BufResult<(), T> {
...
pub trait AsyncWriteAtExt: AsyncWriteAt {
    ...
    async fn write_all_at<T: IoBuf + Send>(&mut self, mut buf: T, pos: u64) -> BufResult<(), T> {
        loop_write_all!(
            buf,
            buf.buf_len(),
            needle,
    ...
    async fn write_vectored_all_at<T: IoVectoredBuf + Send>(
        &mut self,
        mut buf: T,
        pos: u64,
    ) -> BufResult<(), T> {
