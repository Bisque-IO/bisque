...
pub trait AsyncWriteAt {
    ...
    /// position.
    async fn write_at<T: IoBuf + Send>(&mut self, buf: T, pos: u64) -> BufResult<usize, T>;

    /// Like [`AsyncWrite::write_vectored`], except that it writes at a
    /// specified position.
    async fn write_vectored_at<T: IoVectoredBuf + Send>(
        &mut self,
        buf: T,
        pos: u64,
    ) -> BufResult<usize, T> {
    ...
    ($($(const $len:ident =>)? $ty:ty),*) => {
        $(
            impl<$(const $len: usize)?> AsyncWriteAt for $ty {
                ...
                async fn write_vectored_at<T: IoVectoredBuf + Send>(&mut self, buf: T, pos: u64) -> BufResult<usize, T> {
                    let mut iter = match buf.owned_iter() {
                        Ok(buf) => buf,
                        Err(buf) => return BufResult(Ok(0), buf),
                    };
