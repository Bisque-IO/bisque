...
use crate::buf::{BufResult, IoBufMut, IoVectoredBufMut};
use crate::{buf_try, t_alloc};
...
use super::util::{slice_to_buf, slice_to_uninit};
...
pub trait AsyncRead {
    ...
    ///
    /// [`SetLen::set_len`]: crate::buf::SetLen::set_len
...
impl<A: AsyncRead + ?Sized> AsyncRead for &mut A {
    #[inline(always)]
    async fn read<T: IoBufMut + Send>(&mut self, buf: T) -> BufResult<usize, T> {
        (**self).read(buf).await
    }

    #[inline(always)]
    async fn read_vectored<T: IoVectoredBufMut + Send>(&mut self, buf: T) -> BufResult<usize, T> {
...
impl AsyncRead for &[u8] {
    #[inline]
    async fn read<T: IoBufMut + Send>(&mut self, mut buf: T) -> BufResult<usize, T> {
        let len = slice_to_buf(self, &mut buf);
        *self = &self[len..];
        BufResult(Ok(len), buf)
    }

    async fn read_vectored<T: IoVectoredBufMut + Send>(
        &mut self,
        mut buf: T,
    ) -> BufResult<usize, T> {
...
pub trait AsyncReadAt {
    ...
    async fn read_at<T: IoBufMut + Send>(&self, buf: T, pos: u64) -> BufResult<usize, T>;

    /// Like [`AsyncRead::read_vectored`], except that it reads at a specified
    /// position.
    async fn read_vectored_at<T: IoVectoredBufMut + Send>(
        &self,
        buf: T,
        pos: u64,
    ) -> BufResult<usize, T> {
    ...
    (@ptr $($ty:ty),*) => {
        $(
            impl<A: AsyncReadAt + ?Sized> AsyncReadAt for $ty {
                ...
                async fn read_vectored_at<T: IoVectoredBufMut + Send>(&self, buf: T, pos: u64) -> BufResult<usize, T> {
    ...
    (@slice $($(const $len:ident =>)? $ty:ty), *) => {
        $(
            impl<$(const $len: usize)?> AsyncReadAt for $ty {
                async fn read_at<T: IoBufMut + Send>(&self, mut buf: T, pos: u64) -> BufResult<usize, T> {
                    let pos = pos.min(self.len() as u64);
                    let len = slice_to_buf(&self[pos as usize..], &mut buf);
                    BufResult(Ok(len), buf)
                }

                async fn read_vectored_at<T: IoVectoredBufMut + Send>(&self, mut buf: T, pos: u64) -> BufResult<usize, T> {
