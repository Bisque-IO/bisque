...
use crate::buf::{BufResult, IntoInner, IoBuf, IoBufMut, IoVectoredBufMut, Uninit};
use crate::t_alloc;

use super::super::{AsyncRead, AsyncReadAt, IoResult, util::Take};
    ...
    ($t:ty, $be:ident, $le:ident) => {
        ::paste::paste! {
            ...
            async fn [< read_ $t >](&mut self) -> IoResult<$t> {
                use crate::buf::{arrayvec::ArrayVec, BufResult};
...
/// Macro for looping over vectored read operations.
#[macro_export]
macro_rules! loop_read_vectored {
    ($buf:ident, $iter:ident, $read_expr:expr) => {{
        ...
        let mut $iter = match $buf.owned_iter() {
            Ok(buf) => buf,
            Err(buf) => return $crate::buf::BufResult(Ok(0), buf),
        };
...
pub trait AsyncReadExt: AsyncRead {
    ...
    /// area.
    async fn append<T: IoBufMut + Send>(&mut self, buf: T) -> BufResult<usize, T> {
        self.read(buf.uninit()).await.map_buffer(Uninit::into_inner)
    }

    /// Read the exact number of bytes required to fill the buf.
    async fn read_exact<T: IoBufMut + Send>(&mut self, mut buf: T) -> BufResult<(), T> {
    ...
    async fn read_to_end<#[cfg(feature = "allocator_api")] A: Allocator + 'static + Send>(
        &mut self,
    ...
    async fn read_vectored_exact<T: IoVectoredBufMut + Send>(
        &mut self,
        mut buf: T,
    ) -> BufResult<(), T> {
...
pub trait AsyncReadAtExt: AsyncReadAt {
    ...
    async fn read_exact_at<T: IoBufMut + Send>(&self, mut buf: T, pos: u64) -> BufResult<(), T> {
        loop_read_exact!(
            buf,
            buf.buf_capacity(),
            read,
    ...
    async fn read_to_end_at<#[cfg(feature = "allocator_api")] A: Allocator + 'static + Send>(
        &self,
    ...
    /// specified position.
    async fn read_vectored_exact_at<T: IoVectoredBufMut + Send>(
        &self,
        mut buf: T,
        pos: u64,
    ) -> BufResult<(), T> {
