...
impl SocketOpts {
    ...
    pub(crate) fn setup_socket(&self, socket: &Socket) -> std::io::Result<()> {
        if let Some(size) = self.recv_buffer_size {
            socket.inner.set_recv_buffer_size(size)?;
        }
        if let Some(size) = self.send_buffer_size {
            socket.inner.set_send_buffer_size(size)?;
        }
        if let Some(keepalive) = self.keepalive {
            socket.inner.set_keepalive(keepalive)?;
        }
        if let Some(linger) = self.linger {
            socket.inner.set_linger(Some(linger))?;
        }
        if let Some(read_timeout) = self.read_timeout {
            socket.inner.set_read_timeout(Some(read_timeout))?;
        }
        if let Some(write_timeout) = self.write_timeout {
            socket.inner.set_write_timeout(Some(write_timeout))?;
        }
        if let Some(reuse_address) = self.reuse_address {
            socket.inner.set_reuse_address(reuse_address)?;
        }
        // reuse_port is not available in socket2 0.6 - would need platform-specific code
        #[cfg(all(
            unix,
            not(any(target_os = "illumos", target_os = "solaris", target_os = "cygwin"))
        ))]
        if let Some(_reuse_port) = self.reuse_port {
            // socket.inner.set_reuse_port(reuse_port)?;
            // TODO: Implement via setsockopt if needed
        }
        if let Some(nodelay) = self.nodelay {
            socket.inner.set_tcp_nodelay(nodelay)?;
        }
        Ok(())
    }
}
