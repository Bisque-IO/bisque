...
impl FlatMessageBuilder {
    ...
    pub fn build(self) -> Bytes {
        ...
        // -- Write span index --
        // We'll collect spans first, then write them, then write data.
        struct Span {
            offset: u32,
            length: u32,
            data: Bytes,
...
// ---------------------------------------------------------------------------
// Conversion from MessagePayload
// ---------------------------------------------------------------------------

use crate::types::MessagePayload;

impl From<&MessagePayload> for FlatMessageBuilder {
    fn from(msg: &MessagePayload) -> Self {
        let mut builder = FlatMessageBuilder::new(msg.value.clone()).timestamp(msg.timestamp);

        if let Some(ref key) = msg.key {
            builder = builder.key(key.clone());
        }
        if let Some(ttl) = msg.ttl_ms {
            builder = builder.ttl_ms(ttl);
        }
        if let Some(delay) = msg.delay_ms {
            builder = builder.delay_ms(delay);
        }
        if let Some(ref rk) = msg.routing_key {
            builder = builder.routing_key(Bytes::from(rk.clone()));
        }
        if let Some(ref rt) = msg.reply_to {
            builder = builder.reply_to(Bytes::from(rt.clone()));
        }
        if let Some(ref cid) = msg.correlation_id {
            builder = builder.correlation_id(Bytes::from(cid.clone()));
        }
        for (k, v) in &msg.headers {
            builder = builder.header(Bytes::from(k.clone()), v.clone());
        }
        builder
    }
}
...
impl FlatMessage {
    /// Convert back to a `MessagePayload`. This allocates — use only at
    /// protocol adapter boundaries, not in the hot path.
    pub fn to_message_payload(&self) -> MessagePayload {
        MessagePayload {
            key: self.key(),
            value: self.value(),
            headers: self
                .headers()
                .map(|(k, v)| (String::from_utf8(k.to_vec()).unwrap_or_default(), v))
                .collect(),
...
mod tests {
    ...
    fn message_payload_roundtrip() {
        let payload = MessagePayload {
            ...
            headers: vec![("h1".into(), Bytes::from_static(b"hv1"))],
            timestamp: 42,
            ttl_ms: Some(100),
        ...
        let builder: FlatMessageBuilder = (&payload).into();
        ...
        assert_eq!(back.headers.len(), payload.headers.len());
        assert_eq!(back.headers[0].0, "h1");
        assert_eq!(back.headers[0].1, Bytes::from_static(b"hv1"));
    }
