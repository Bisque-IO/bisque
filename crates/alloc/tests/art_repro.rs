...
fn art_single_thread_leak() {
    for round in 0..30_000 {
        ...
        let g = tree.pin();
        for k in 0..100usize { let _ = tree.insert(k, k, &g); }
        for k in 0..50usize { tree.remove(&k, &g); }
        drop(g);
        drop(tree);
        drop(master);
        if round % 5000 == 0 {
...
fn art_concurrent_leak() {
    ...
    for round in 0..30_000 {
        let master = HeapMaster::new(4 * 1024 * 1024).unwrap();
        let tree = Arc::new(bisque_alloc::collections::art::Art::<usize, usize>::new(&master));
        ...
        let writer = std::thread::spawn(move || {
            ...
            while !done2.load(Ordering::Relaxed) {
                ...
                for _ in 0..64 {
                    let k = i % 200;
                    if i % 2 == 0 { let _ = tree2.insert(k, k, &g); }
                    else { tree2.remove(&k, &g); }
                    i += 1;
                }
            }
        });
        ...
        let g = tree.pin();
        for k in 0..200usize { let _ = tree.insert(k, k, &g); }
        for k in 0..100usize { tree.remove(&k, &g); }
        drop(g);
