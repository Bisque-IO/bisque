...
void _mi_theap_init(mi_theap_t* theap, mi_heap_t* heap, mi_tld_t* tld)
{
  ...
  // Memlimit heaps must not abandon pages. Abandoned pages cannot be cleanly
  // attributed to a single heap (they can be reclaimed by any heap), breaking
  // memory limit tracking. With abandonment disabled, full pages stay in the
  // heap's full queue and remain tracked in memory_committed.
  if (mi_heap_has_memlimit(heap)) {
    theap->allow_page_abandon = false;
  }
  #endif
...
void _mi_theap_delete(mi_theap_t* theap)
{
  ...
  // Abandon all pages. For memlimit heaps, abandoned pages retain their
  // page->heap affinity — the reclaim path checks this to prevent
  // cross-heap page stealing (see mi_arenas_page_heap_match).
