...
// Push a page onto a heap queue. Returns true on success.
// With MI_HEAP_MEMLIMIT, returns false if adding this page would exceed the heap's memory limit.
...
static void mi_page_queue_move_to_front(mi_heap_t* heap, mi_page_queue_t* queue, mi_page_t* page) {
  ...
  // Unlink from current position (without memory_usage accounting — page stays in same heap)
  if (page->prev != NULL) page->prev->next = page->next;
  if (page->next != NULL) page->next->prev = page->prev;
  if (page == queue->last) queue->last = page->prev;
  // Link at front
