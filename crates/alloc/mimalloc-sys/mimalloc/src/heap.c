...
void mi_heap_set_memory_limit(mi_heap_t* heap, size_t max_bytes) {
  if (heap == NULL) return;
  mi_atomic_storei64_relaxed(&heap->memory_limit, (int64_t)max_bytes);
  // Disable page abandonment on all existing theaps. Memlimit heaps require
  // pages to stay in theap queues for accurate tracking.
  if (max_bytes > 0) {
...
size_t mi_heap_get_memory_limit(mi_heap_t* heap) {
  ...
  return (size_t)mi_atomic_loadi64_relaxed(&heap->memory_limit);
}

size_t mi_heap_get_memory_usage(mi_heap_t* heap) {
  if (heap == NULL) return 0;
  const int64_t v = mi_atomic_loadi64_relaxed(&heap->memory_committed);
