...
static int mi_page_commit_usage(mi_page_t* page) {
  // if ((size_t)_mi_page_mem_slices(page) * MI_ARENA_SLICE_SIZE <= MI_PAGE_MIN_COMMIT_SIZE) return 100;
...
static bool mi_heap_delete_page(const mi_heap_t* heap, const mi_heap_area_t* area, void* block, size_t block_size, void* arg) {
  ...
  if (page->used==0) {
    ...
    // Note: memlimit heaps have allow_page_abandon=false, so pages from a
    // memlimit heap never reach this transfer path. The page was already
    // removed from its queue (decrementing memory_committed) above.
