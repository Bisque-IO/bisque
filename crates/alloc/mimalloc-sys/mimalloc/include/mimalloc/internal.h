...
#if defined(MI_HEAP_MEMLIMIT)
// Get the page memory in arena slice units (for memory limit tracking).
static inline uint32_t _mi_page_mem_slices(const mi_page_t* page) {
  if (page->memid.memkind == MI_MEM_ARENA) {
    return page->memid.mem.arena.slice_count;
  }
    ...
    const size_t size = (presize > (ptrdiff_t)page->memid.mem.os.size ? 0 : page->memid.mem.os.size - (size_t)presize);
    return (uint32_t)((size + MI_ARENA_SLICE_SIZE - 1) / MI_ARENA_SLICE_SIZE);
  }
  else {
    return 0;
  }
...
// Queue-membership memlimit helpers.
// `page_bytes` is the committed size of the page being added to a theap's queue.
// Returns true if adding the page would exceed the heap's memory limit.
// On success, atomically increments memory_committed.
static inline bool mi_heap_memlimit_page_over(mi_heap_t* heap, size_t page_bytes) {
  if mi_likely(heap == NULL) return false;
  const int64_t limit = mi_atomic_loadi64_relaxed(&heap->memory_limit);
  if mi_likely(limit <= 0) return false;  // unlimited
  // fetch_add returns the OLD value; the new value is old + page_bytes
  const int64_t old_usage = mi_atomic_addi64_relaxed((volatile int64_t*)&heap->memory_committed, (int64_t)page_bytes);
  if mi_likely(old_usage + (int64_t)page_bytes <= limit) return false;  // under limit
  // over limit — undo the increment
  mi_atomic_addi64_relaxed((volatile int64_t*)&heap->memory_committed, -(int64_t)page_bytes);
  return true;
}

// Decrement memory usage when a page is removed from a theap's queue.
static inline void mi_heap_memlimit_page_free(mi_heap_t* heap, size_t page_bytes) {
  if mi_likely(heap == NULL) return;
  if mi_likely(mi_atomic_loadi64_relaxed(&heap->memory_limit) <= 0) return;
  mi_atomic_addi64_relaxed((volatile int64_t*)&heap->memory_committed, -(int64_t)page_bytes);
}

// Check if this heap has a memory limit set (used to skip force-collect on OOM).
