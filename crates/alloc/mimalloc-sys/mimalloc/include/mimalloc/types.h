...
typedef struct mi_heap_s {
  ...
  // Queue-membership memory limit tracking.
  // memory_limit: max bytes (0 = unlimited). Set once, read with relaxed semantics.
  // memory_committed: total committed bytes of pages currently in theap queues.
  //   Incremented in mi_page_queue_push / push_at_end (page enters queue).
  //   Decremented in mi_page_queue_remove (page leaves queue — freed or abandoned).
  //   Memlimit heaps set allow_page_abandon=false so full pages stay in the
  //   full queue (MI_BIN_FULL) and remain tracked. On theap exit, pages are
  //   abandoned (decremented) but retain per-heap affinity for reclaim.
  _Atomic(int64_t)      memory_limit;      // max bytes (0 = unlimited)
  _Atomic(int64_t)      memory_committed;  // pages in theap queues (bytes)
  #endif
} mi_heap_t;
