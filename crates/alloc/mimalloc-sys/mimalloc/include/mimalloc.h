...
// Set a memory limit on a heap. When the limit is reached, allocations return NULL.
// Set to 0 for unlimited. Tracked at slice granularity (64 KiB).
mi_decl_export void mi_heap_set_memory_limit(mi_heap_t* heap, size_t max_bytes);

// Get the memory limit of a heap (in bytes, 0 = unlimited).
mi_decl_export size_t mi_heap_get_memory_limit(mi_heap_t* heap);

// Get the current memory usage of a heap (in bytes, tracked at slice granularity).
mi_decl_export size_t mi_heap_get_memory_usage(mi_heap_t* heap);
#endif
