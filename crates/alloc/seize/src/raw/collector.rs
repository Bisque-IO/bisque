...
impl Collector {
    ...
    pub unsafe fn reclaim_all(&self) {
        // First: drain all reservation lists. Submitted batches live on
        // reservation lists and are only freed when their ref-count reaches
        // zero (via `traverse`). Since no threads are active (safety
        // contract), we can forcibly drain every reservation and free all
        // remaining batches regardless of ref-count.
        //
        // Safety: The caller guarantees no threads are accessing the
        // collector, so we can safely read and modify all reservations.
        for reservation in unsafe { self.reservations.iter() } {
            let head = reservation.head.swap(Entry::INACTIVE, Ordering::Relaxed);
            if !head.is_null() && head != Entry::INACTIVE {
                // Traverse the list and force-free each batch.
                let mut list = head;
                while !list.is_null() {
                    let curr = list;
                    list = unsafe { (*curr).state.next };
                    let batch = unsafe { (*curr).batch };
                    // Decrement ref-count. If it reaches zero we free.
                    // If it doesn't (other reservations still reference it),
                    // we'll catch it on their iteration.
                    unsafe {
                        if (*batch).active.fetch_sub(1, Ordering::Release) == 1 {
                            atomic::fence(Ordering::Acquire);
                            self.free_batch(batch);
        ...
        // Second: free any remaining local (unsubmitted) batches.
        // Safety: Guaranteed by caller.
