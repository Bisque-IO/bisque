...
impl<V: Clone + From<usize> + Into<usize>> U64Congee<V> {
    ...
    pub fn get(&self, key: u64, guard: &impl epoch::Guard) -> Option<V> {
    ...
    pub fn remove(&self, key: u64, guard: &impl epoch::Guard) -> Option<V> {
        let key: [u8; 8] = key.to_be_bytes();
    ...
    usize: From<V>,
{
    ...
    /// ```
    #[inline]
    pub fn get(&self, key: &K, guard: &impl epoch::Guard) -> Option<V> {
    ...
    /// ```
    #[inline]
    pub fn remove(&self, k: &K, guard: &impl epoch::Guard) -> Option<V> {
    ...
    /// ```
    #[inline]
    pub fn insert(&self, k: K, v: V, guard: &impl epoch::Guard) -> Result<Option<V>, OOMError> {
        let key = usize::from(k);
        let key: [u8; 8] = key.to_be_bytes();
