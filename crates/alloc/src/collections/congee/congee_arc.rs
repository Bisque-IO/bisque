...
use super::{DefaultAllocator, RawCongee, epoch, error::OOMError};
...
/// Seize-compatible reclaim function that reconstructs and drops an [`Arc<V>`].
///
/// # Safety
    ...
    usize: From<K>,
{
    ...
    /// let guard = tree.pin();
    /// ```
    pub fn pin(&self) -> epoch::LocalGuard<'_> {
        self.inner.pin()
    }
    ...
    /// assert!(!tree.is_empty(&guard));
    /// ```
    pub fn is_empty(&self, guard: &impl epoch::Guard) -> bool {
    ...
    /// assert!(tree.is_empty(&guard));
    /// ```
    pub fn remove(&self, key: K, guard: &impl epoch::Guard) -> Option<Arc<V>> {
        ...
        let delayed_ptr = Arc::into_raw(delayed_v) as *mut V;
        unsafe {
            guard.defer_retire(delayed_ptr, reclaim_arc::<V>);
        }
        Some(rt)
    }
    ...
    /// ```
    pub fn get(&self, key: K, guard: &impl epoch::Guard) -> Option<Arc<V>> {
        let usize_key: usize = usize::from(key);
        let key: [u8; 8] = usize_key.to_be_bytes();
...
mod tests {
    ...
    fn test_reference_counting() {
        ...
        let old = tree.insert(1, counter.clone(), &guard).unwrap().unwrap();
        // counter(1) + tree(1) + old(1) + deferred(1) = 4
        assert_eq!(Arc::strong_count(&counter), 4);
        {
            drop(guard);
            drop(old);
            // With seize, deferred retirements are reclaimed asynchronously.
            // After dropping the tree, reclaim_all is not safe on a shared
            // collector, but the deferred Arc will eventually be reclaimed.
            // We verify the count is at most 3 (counter + tree + possibly deferred).
            assert!(Arc::strong_count(&counter) <= 3);
        }

        let guard = tree.pin();
        let removed = tree.remove(1, &guard).unwrap();
        // counter(1) + removed(1) + deferred-from-remove(1) + possibly-old-deferred(1)
        // Count is 3 or 4 depending on whether the previous deferred was reclaimed.
        let count = Arc::strong_count(&counter);
        assert!(count == 3 || count == 4, "unexpected count: {count}");
        drop(removed);
        // counter(1) + deferred(s) still pending
        let count = Arc::strong_count(&counter);
        assert!(
            count >= 1 && count <= 3,
            "unexpected count after drop: {count}"
        );
    }
