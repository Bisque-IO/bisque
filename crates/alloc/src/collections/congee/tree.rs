...
/// Seize-compatible reclaim function for [`RetireCtx`].
///
/// # Safety
...
impl<const K_LEN: usize, A: Allocator + Clone + Send> RawCongee<K_LEN, A> {
    pub(crate) fn is_empty(&self, _guard: &impl Guard) -> bool {
        loop {
    ...
    #[inline]
    pub(crate) fn get(&self, key: &[u8; K_LEN], _guard: &impl Guard) -> Option<usize> {
        'outer: loop {
            let mut level = 0;
    ...
    pub(crate) fn value_count(&self, _guard: &impl Guard) -> usize {
        loop {
                    ...
                    match new_v {
                        ...
                        None => {
                            ...
                            if node.as_ref().value_count() == 1 {
                                ...
                                let node_ptr = NonNull::from(write_n.as_mut());
                                std::mem::forget(write_n);
                                let layout = std::alloc::Layout::new::<RetireCtx<A>>();
                                let raw =
                                    self.allocator.allocate(layout).map_err(|_| ArtError::Oom)?;
                                ...
                                unsafe {
                                    ...
                                    guard.defer_retire(ctx, reclaim_node_ctx::<A>);
                                }
                            } else {
                                let mut write_n = node.upgrade().map_err(|(_n, v)| v)?;
