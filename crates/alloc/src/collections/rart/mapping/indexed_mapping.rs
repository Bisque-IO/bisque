...
impl<N, const WIDTH: usize, Bitset: BitsetTrait> IndexedMapping<N, WIDTH, Bitset> {
    pub fn new(heap: &Heap) -> Result<Self, AllocError> {
        Ok(Self {
            child_ptr_indexes: BitArray::new(heap)?,
            children: BitArray::new(heap)?,
            num_children: 0,
        })
    }

    pub(crate) fn from_direct(dm: &mut DirectMapping<N>, heap: &Heap) -> Result<Self, AllocError> {
        let mut indexed = IndexedMapping::new(heap)?;
    ...
    pub fn from_sorted_keyed<const KM_WIDTH: usize>(
        ...
        let mut im: IndexedMapping<N, WIDTH, Bitset> = IndexedMapping::new(heap)?;
...
mod test {
    ...
    fn test_basic_mapping() {
        ...
        let mut mapping = super::IndexedMapping::<u8, 48, Bitset16<3>>::new(&heap).unwrap();
        for i in 0..48 {
