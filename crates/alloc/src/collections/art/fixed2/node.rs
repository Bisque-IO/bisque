...
//!
//! This is a variant of `crate::collections::art::node` where the key size
//! is a compile-time constant `K` instead of being fixed to `size_of::<usize>()`.
//! `LeafNode<K, V>` stores the key inline as `[u8; K]` and the value as `V`.
...
/// Compare two `[u8; K]` keys using the widest native word size.
/// For K=4,8,16 this compiles to 1-2 register comparisons instead of memcmp.
#[inline(always)]
pub(crate) fn keys_eq<const K: usize>(a: &[u8; K], b: &[u8; K]) -> bool {
    // Let the compiler auto-vectorize based on K.
    // For K <= 8: single u64 compare. K=16: two u64 compares.
    // The key insight: compare via raw pointer cast to avoid the
    // reference-dereference overhead of [u8; K] == [u8; K].
    let ap = a.as_ptr();
    let bp = b.as_ptr();
    let mut i = 0;
    // Compare 8 bytes at a time.
    while i + 8 <= K {
        let av = unsafe { (ap.add(i) as *const u64).read_unaligned() };
        let bv = unsafe { (bp.add(i) as *const u64).read_unaligned() };
        if av != bv { return false; }
        i += 8;
    }
    // Compare 4 bytes.
    if i + 4 <= K {
        let av = unsafe { (ap.add(i) as *const u32).read_unaligned() };
        let bv = unsafe { (bp.add(i) as *const u32).read_unaligned() };
        if av != bv { return false; }
        i += 4;
    }
    // Compare remaining bytes.
    while i < K {
        if unsafe { *ap.add(i) != *bp.add(i) } { return false; }
        i += 1;
    }
    true
}
...
#[inline]
pub(crate) fn leaf_ptr<const K: usize, V>(p: usize) -> *mut LeafNode<K, V> {
    (p & !LEAF_TAG) as *mut LeafNode<K, V>
}

#[inline]
pub(crate) fn tag_leaf<const K: usize, V>(p: *mut LeafNode<K, V>) -> usize {
    (p as usize) | LEAF_TAG
}

#[repr(C)]
pub(crate) struct LeafNode<const K: usize, V> {
    pub(crate) key: [u8; K],
    pub(crate) value: V,
}
...
pub(crate) unsafe fn alloc_leaf<const K: usize, V>(
    heap: &Heap,
    key: &[u8; K],
    value: V,
) -> usize {
    ...
    unsafe {
        ...
        std::ptr::addr_of_mut!((*leaf).key)
            .cast::<u8>()
            .copy_from_nonoverlapping(key.as_ptr(), K);
...
pub(crate) unsafe fn dealloc_leaf_raw<const K: usize, V>(heap: &Heap, tagged: usize) {
...
pub(crate) unsafe fn dealloc_node<const K: usize, V>(heap: &Heap, p: usize) {
    if p == NULL_CHILD {
        return;
    }
    if is_leaf(p) {
...
pub(crate) unsafe fn free_subtree<const K: usize, V>(heap: &Heap, p: usize) {
    if p == NULL_CHILD {
        return;
    }
    if is_leaf(p) {
