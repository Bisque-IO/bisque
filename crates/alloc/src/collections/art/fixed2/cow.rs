...
/// = K (ensure_mutable per level) + 1 (leaf) + 2 (collapse) + 1 (shrink).
/// Pre-reserving this before mutation guarantees all pushes succeed.
pub(crate) const fn max_garbage_per_op<const K: usize>() -> usize {
    K + 4
}
...
pub(crate) unsafe fn ensure_mutable<const K: usize>(
    node: usize,
    generation: u64,
    heap: &Heap,
...
#[allow(dead_code)]
pub(crate) unsafe fn cow_insert<const K: usize, V>(
    root: usize,
    key: &[u8; K],
    value: V,
    generation: u64,
    ...
    if is_leaf(root) {
        ...
        n4.header.prefix[..stored_prefix].copy_from_slice(&key[..stored_prefix]);
        if common < K {
            unsafe {
...
pub(crate) unsafe fn cow_insert_inner<const K: usize, V>(
    node: usize,
    key: &[u8; K],
    value: V,
    depth: usize,
    ...
    if is_leaf(child) {
        ...
        n4.header.prefix_len = common as u32;
        n4.header.prefix[..stored_common]
            .copy_from_slice(&key[next_depth..next_depth + stored_common]);
...
#[allow(dead_code)]
pub(crate) unsafe fn cow_remove<const K: usize, V>(
    root: usize,
    key: &[u8; K],
    generation: u64,
    heap: &Heap,
...
pub(crate) unsafe fn cow_remove_inner<const K: usize, V>(
    node: usize,
    key: &[u8; K],
    depth: usize,
    generation: u64,
