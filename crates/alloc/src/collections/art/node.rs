//! Node types and operations for the Adaptive Radix Tree.
...
/// Maximum key depth (bytes in a usize).
pub(crate) const KEY_BYTES: usize = std::mem::size_of::<usize>();
...
#[inline]
pub(crate) fn leaf_ptr(p: usize) -> *mut LeafNode {
    (p & !LEAF_TAG) as *mut LeafNode
}

#[inline]
pub(crate) fn tag_leaf(p: *mut LeafNode) -> usize {
    (p as usize) | LEAF_TAG
}
...
/// Extract all key bytes as an array.
#[inline]
pub(crate) fn key_bytes_array(key: usize) -> [u8; KEY_BYTES] {
    key.to_be_bytes()
}
...
pub(crate) unsafe fn alloc_leaf(heap: &Heap, key: usize, value: usize) -> usize {
    let layout = std::alloc::Layout::new::<LeafNode>();
    let ptr = heap.alloc(layout.size(), layout.align());
    if ptr.is_null() {
        return NULL_CHILD;
    }
    unsafe {
        (ptr as *mut LeafNode).write(LeafNode { key, value });
    }
    tag_leaf(ptr as *mut LeafNode)
}
...
/// Deallocate a node (inner or leaf).
pub(crate) unsafe fn dealloc_node(heap: &Heap, p: usize) {
    if p == NULL_CHILD {
        return;
    }
    if is_leaf(p) {
        unsafe { heap.dealloc(leaf_ptr(p) as *mut u8) };
    } else {
...
pub(crate) unsafe fn free_subtree(heap: &Heap, p: usize) {
    if p == NULL_CHILD {
        return;
    }
    if is_leaf(p) {
    ...
    match hdr.kind {
        ...
        NodeKind::N48 => {
            ...
            for byte in 0..256usize {
                let idx = n.child_index[byte];
                if idx != EMPTY_INDEX {
                    unsafe { free_subtree(heap, n.children[idx as usize]) };
                }
            }
        }
        NodeKind::N256 => {
