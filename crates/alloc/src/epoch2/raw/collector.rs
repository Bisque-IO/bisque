...
impl Collector {
    ...
    pub unsafe fn add<T>(
        &self,
        ptr: *mut T,
        reclaim: unsafe fn(*mut T, &crate::epoch::collector::Collector),
        thread: Thread,
    ) {
        ...
        if batch == LocalBatch::DROP {
            ...
            // provided reclaimer.
            unsafe { reclaim(ptr, crate::epoch::collector::Collector::from_raw(self)) }
            return;
        }

        // Safety: `fn(*mut T) and fn(*mut U)` are ABI compatible if `T, U: Sized`.
        let reclaim: unsafe fn(*mut (), &crate::epoch::collector::Collector) =
    ...
    unsafe fn free_batch(&self, batch: *mut Batch) {
        ...
        for entry in unsafe { (*batch).entries.iter_mut() } {
            unsafe { (entry.reclaim)(entry.ptr.cast(), crate::epoch::collector::Collector::from_raw(self)) };
        }
...
struct Entry {
    ...
    reclaim: unsafe fn(*mut (), &crate::epoch::collector::Collector),
